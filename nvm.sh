      ...
      if [ $nobinary -ne 1 ]; then
        ...
        if [ -n "$os" ]; then
          ...
          if [ $binavail -eq 1 ]; then
            if [ "$os" = "mingw" ]; then
              t="$VERSION-$os-$arch"
              url="http://nodejs.org/dist/$VERSION/node.exe"
              local reg
              if [ "$arch" = "x64" ]; then
                reg=' x64/node.exe'
              else
              ...
              sum=`curl -s http://nodejs.org/dist/$VERSION/SHASUMS.txt | \grep "$reg" | awk '{print $1}'`
              local tmpdir="$NVM_DIR/bin/node-${t}"
              local tmpexe="$tmpdir/node.exe"
              if (
                mkdir -p "$tmpdir" && \
                curl -L -C - --progress-bar $url -o "$tmpexe" && \
                nvm_checksum `${shasum} "$tmpexe" | awk '{print $1}'` $sum && \
                mkdir -p "$NVM_DIR/$VERSION/bin" && \
                mv "$tmpexe" "$NVM_DIR/$VERSION/bin"
                )
              then
                nvm use $VERSION
                rm -rf "$tmpdir"
